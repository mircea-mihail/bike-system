...
def check_for_gw(p_img, p_chunk, p_label_mat, p_hsv_img):
...
def fill_in_shape(p_img, p_label_mat, p_hsv_img, p_label, p_x, p_y):
    ...
    if chunk_size >= MIN_CHUNK_SIZE:
        chunk_score = check_for_gw(p_img, gw_chunk, p_label_mat, p_hsv_img)
        if chunk_score > MIN_CHUNK_SCORE:
            # print("similitude:", chunk_score)
            return chunk_score, gw_chunk, chunk_score 
    
    return 0, 0, 0


def detect_gw(p_img):
    hsv_image = cv.cvtColor(p_img, cv.COLOR_RGB2HSV)
    ...
    for i in range(1, len(p_img) - 2):
        for j in range(1, len(p_img[0]) - 2):
            if is_red(hsv_image[i][j]) and label_mat[i][j] == 0:
                current_score, gw_chunk, chunk_score = fill_in_shape(p_img, label_mat, hsv_image, current_label, j, i)

                # just for printing, normally return true
                if(current_score > 0):
    ...
    # second for print
    return num_detections, label_mat
